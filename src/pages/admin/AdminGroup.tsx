...
function AdminGroup() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let apiUrl = 'http://localhost:3000/api/v1/group?orderBy=popularity'; // 기본적으로 인기순 API 호출

        if (selectedSort === '최근순') {
          apiUrl = 'http://localhost:3000/api/v1/group'; // 최신순 API 호출
        }
  ...
  async function fetchAllGroup() {
    try {
      const response = await axios.get('http://localhost:3000/api/v1/group');
      return response.data.data;
    } catch (error) {
      throw error;
    }
